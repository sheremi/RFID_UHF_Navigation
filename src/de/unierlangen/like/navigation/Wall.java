...
public class Wall extends Obstacle {
	...
	public Wall(float x1, float y1, float x2, float y2) {
		...
		this.x2_wall = x2;
		this.y2_wall = y2;
		//the slope ("k" in the Linear equation "y=kx+b") or gradient of the wall's line
	...
	/**
	 * Finds out, if the wall is located between a tag and a possible position of the reader.
	 * If yes, calculates intersections of the wall with the line between tag and position.
	 * @param x Coordinate x of the possible reader's position
	 * @param y Coordinate y of the possible reader's position
	 * @param tag Object of the regarded tag 
	...
	public PointF getIntersection (Tag tag, float x, float y){
		...
		if ((xWall_min<=x_cross) && (xWall_max>=x_cross) && (xTagPoint_min<=x_cross) && (xTagPoint_max>=x_cross) &&
			(yWall_min<=y_cross) && (yWall_max>=y_cross) && (yTagPoint_min<=y_cross) && (yTagPoint_max>=y_cross)) {
